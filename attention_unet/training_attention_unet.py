...
log_filename = f"./retrained_L93_{domain}_{vertical}_{features}_smalldropout_epoch_{init_epoch}_to_{init_epoch+nepochs-1}.txt"
...
if vertical == "stratosphere_only":
    ...
    # L93 CAM7 version
    pre = "/scratch/users/ag4680/retrain_nonlocal_training_data_global_L93/1x1_inputfeatures_u_v_theta_w_uw_vw_gcp_era5_training_data_hourly_"
...
for year in train_years:
    ...
    for months in np.arange(1, 13):
        ...
        train_files.append(f"{pre}{year}_L93_constant_mu_sigma_scaling{str(months).zfill(2)}.nc")
...
for year in test_years:
    for months in np.arange(1, 13):
        ...
        test_files.append(f"{pre}{year}_L93_constant_mu_sigma_scaling{str(months).zfill(2)}.nc")

#print(test_files)

write_log(
    f"Re-training an L93 Attention UNet {domain} horizontal and {vertical} vertical model, with features {features} with min-max learning rates {lr_min} to {lr_max}, and dropout={dropout}. Starting from epoch {init_epoch}. Training on years {train_years} and testing on years {test_years}."
)
...
model = Attention_UNet(ch_in=ch_in, ch_out=ch_out, dropout=dropout, attn_3d=False)
...
file_prefix = f"/scratch/users/ag4680/torch_saved_models/attention_unet_L93/retrained_L93_attnunet_era5_{domain}_{vertical}_{features}_mseloss"
if restart:
