...
class Dataset_ANN_CNN(torch.utils.data.Dataset):
    def __init__(self, files, domain, vertical, stencil, manual_shuffle, features, region="1andes"):
        ...
        # dimensions
        self.idim = len(self.ds["in_channel"])
        self.odim = len(self.ds["out_channel"])
        self.features = features
        self.inp = self.ds["inputs"]
        self.out = self.ds["targets"]
        ...
        # resolve the channel layout
        input_vars = self.ds.attrs['input_vars'].split('_')      # ordered names
        levels     = {'ps': 1, 'u': 93, 'v': 93, 'theta': 93, 'omega':93}    # channels per var
        assert sum(levels[v] for v in input_vars) == self.idim, "levels != idim"
        # load normalisation stats
        zn = np.load("/scratch/groups/aditis2/cesm_mle_training_data/normalization_statistics/"
            "training_period_means_and_stds_x02_x03_ps_u_v_theta_w_uw_vw.npz",
            allow_pickle=True,
        )
        stats = zn['stats'].item()
        input_mean  = stats['input_mean'].astype('float64')      # (373)
        input_std   = stats['input_stdev'].astype('float64')     # (373)
        output_mean = stats['output_mean'].astype('float64')     # (186)
        output_std  = stats['output_stdev'].astype('float64')    # (186)
        
        print("NORMALISING INPUTS")
        # inputs: per-variable z-score, cbrt only for w/vort ---
        pieces, start = [], 0
        for name in input_vars:
            n = levels[name]
            blk = self.inp.isel(in_channel=slice(start, start + n))

            m = xr.DataArray(input_mean[start:start + n], dims='in_channel', coords={'in_channel': blk['in_channel']})
            s = xr.DataArray(input_std[start:start + n], dims='in_channel', coords={'in_channel': blk['in_channel']})

            z = (blk - m) / s
            if name in cbrt_vars:
                z = xr.apply_ufunc(np.cbrt, z, dask='parallelized', output_dtypes=['float64'])
            pieces.append(z)
            start += n

        self.inp = xr.concat(pieces, dim='in_channel').astype('float32')
        print("INPUT CHANNELS WITHOUT STATIC:")
        print(self.inp)

        # concatenate static variables  
        zs = np.load("/scratch/groups/aditis2/cesm_mle_training_data/static_variables/"
            "static_variables_unscaled.npz")
        static_np = zs['STATIC'].astype('float32') # (8, 192, 288)
        stat = xr.DataArray(
            static_np,
            dims=('in_channel', 'latitude', 'longitude'),
            coords={'latitude': self.lat, 'longitude': self.lon},
        )
        self.inp = xr.concat(
            [stat, self.inp.drop_vars('in_channel')], dim='in_channel')
        self.inp = self.inp.assign_coords(in_channel=np.arange(self.inp.sizes['in_channel']))
        self.inp = self.inp.transpose('time', 'in_channel', 'latitude', 'longitude')   # <-- add this
        print("INPUT CHANNELS WITH STATIC:")
        print(self.inp)

        # targets: all get z-score + cbrt ---
        tm = xr.DataArray(output_mean, dims='out_channel', coords={'out_channel': self.out['out_channel']})
        ts = xr.DataArray(output_std,  dims='out_channel', coords={'out_channel': self.out['out_channel']})
        tz = (self.out - tm) / ts
        self.out = xr.apply_ufunc(np.cbrt, tz, dask='parallelized', output_dtypes=['float64']).astype('float32')
        self.out = self.out.transpose('time', 'out_channel', 'latitude', 'longitude')
        ...
        if self.vertical == "global":
            ...
            if self.features == "uvtheta":
                self.v = np.arange(0, 280)
                #self.v = np.arange(0, 369)  # for u,v,theta # original, working
            elif self.features == "uvthetaw":
                #self.v = np.arange(0, 491)  # for u,v,theta,w
                #self.v = np.arange(0,551) # Aman: retraining for Tom
                self.v = np.arange(0, 381)
            elif self.features == "uvw":
                self.v = np.concatenate(
                    (np.arange(0, 189), np.arange(282, 375)), axis=0
                )
                #self.v = np.concatenate(
...
class Dataset_ANN_CNN_nolatlon(torch.utils.data.Dataset):
    def __init__(self, files, domain, vertical, stencil, manual_shuffle, features, region="1andes"):
        # super().__init__()
        ...
        if self.vertical == "global":
            # 122 channels for each feature
            if self.features == "uvtheta":
                self.v = np.arange(2, 369)  # for u,v,theta # original, working
            elif self.features == "uvthetaw":
                #self.v = np.arange(0, 491)  # for u,v,theta,w
                self.v = np.arange(2,551) # Aman: retraining for Tom
            elif self.features == "uvw":
                self.v = np.concatenate(
                    (np.arange(2, 247), np.arange(369, 491)), axis=0
                )  # for u,v,w
            self.w = np.arange(0, self.odim)  # all vertical channels
            ...
            # 60 channels for each feature
            if self.features == "uvtheta":
                self.v = np.arange(2, 183)  # for u,v,theta
            elif self.features == "uvthetaw":
                self.v = np.arange(2, 243)  # for u,v,theta,w
            elif self.features == "uvw":
                self.v = np.concatenate(
                    (np.arange(2, 123), np.arange(183, 243)), axis=0
                )  # for u,v,w
            elif self.features == "uvthetaN2":
                self.v = np.concatenate(
                    (np.arange(2, 183), np.arange(243, 303)), axis=0
                )  # for u,v,theta,N2
            elif self.features == "uvthetawN2":
                self.v = self.v = np.arange(2, 303)  # for u,v,theta,w,N2
            self.w = np.arange(0, self.odim)  # all vertical channels

        elif self.vertical == "stratosphere_update":
            # 122 channels for each feature
            if self.features == "uvtheta":
                self.v = np.arange(2, 369)  # for u,v,theta
            elif self.features == "uvthetaw":
                self.v = np.arange(2, 491)  # for u,v,theta,w
            elif self.features == "uvw":
                self.v = np.concatenate(
                    (np.arange(2, 247), np.arange(369, 491)), axis=0
                )  # for u,v,w
