...
# model checkpoint
pref = f"/scratch/users/ag4680/torch_saved_models/JAMES_revisions/"#{vertical}/"
ckpt = f"revisions_nolatlon_ann_cnn_{stencil}x{stencil}_{domain}_{vertical}_era5_{features}__train_epoch{epoch}.pt"
...
log_filename = (
    f"./revisions_nolatlon_inference_ann_cnn_{stencil}x{stencil}_{domain}_{vertical}_{features}_ckpt_epoch_{epoch}.txt"
)
...
#test_files=['/scratch/users/ag4680/training_data/era5/1x1_inputfeatures_u_v_theta_w_uw_vw_era5_training_data_hourly_2015_constant_mu_sigma_scalingJul05to20.nc']
#test_years = np.array([2002])
#test_files=[sys.argv[7]]
...
testset = Dataset_ANN_CNN_nolatlon(
    files=test_files,
    domain=domain,
    vertical=vertical,
    stencil=stencil,
...
S = ckpt.split(".")
if dropout == 0:
    out = f"/scratch/users/ag4680/gw_inference_ncfiles/inference_{S[0]}_{test_years[0]}_{test_month}_testcases.nc"
