...
ckpt = f"TLIFS_ann_cnn_K{K}_{stencil}x{stencil}_era5_ifs_{domain}_{vertical}_{features}_mseloss_train_epoch{str(epoch).zfill(2)}.pt"
...
if dropout == 0:
    if teston == "ERA5":
        out = f"/scratch/users/ag4680/gw_inference_ncfiles/TLIFS_inference_K{K}_{S[0]}_{test_years[0]}_{test_month}_testedonERA5.nc"
    else:
        out = f"/scratch/users/ag4680/gw_inference_ncfiles/TLIFS_inference_K{K}_{S[0]}_testedonIFS.nc"
else:
    if teston == "ERA5":
