...
if vertical == "global" or vertical == "stratosphere_update":
    if model_type == "ann":
        log_filename = f"./IFStransfer_{vertical}_ann_cnn_K{K}_{stencil}x{stencil}_{features}_6hl_epoch_{init_epoch}_to_{init_epoch+nepochs-1}.txt"
    elif model_type == "attention":
        log_filename = f"./IFStransfer_{vertical}_attention_K{K}_{features}_epoch_{init_epoch}_to_{init_epoch+nepochs-1}.txt"
...
if model_type == "ann":
    write_log(
        f"Transfer learning: retraining ERA5 trained ANN-CNNs with stencil {stencil}x{stencil}, vertical={vertical}, horizontal={domain} model with features {features}. CyclicLR scheduler to cycle learning rates between lr_min={lr_min} to lr_max={lr_max}, {K=}."
    )
elif model_type == "attention":
    write_log(
        f"Transfer learning: retraining ERA5 trained {model_type}, vertical={vertical}, horizontal={domain} model with features {features}. CyclicLR scheduler to cycle learning rates between lr_min={lr_min} to lr_max={lr_max}, {K=}."
    )
...
model = Model_Freeze_Transfer_Learning(model=model, model_type=model_type, K=K) # K=2, K=3
...
if model_type == "ann":
    file_prefix = f"/scratch/users/ag4680/torch_saved_models/transfer_learning_IFS/ann_cnn/TLIFS_ann_cnn_K{K}_{stencil}x{stencil}_era5_ifs_{domain}_{vertical}_{features}_mseloss"
elif model_type == "attention":
    file_prefix = f"/scratch/users/ag4680/torch_saved_models/transfer_learning_IFS/attention_unet/TLIFS_attnunet_K{K}_era5_ifs_{domain}_{vertical}_{features}_mseloss"
